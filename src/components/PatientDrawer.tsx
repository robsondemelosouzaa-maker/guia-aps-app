                    ...
                    {config.hasBabyBorn && tab === 'dados' && (
                        <div className="mb-4 p-3 bg-pink-50 dark:bg-pink-950/30 border border-pink-100 dark:border-pink-900 rounded-2xl flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`w-12 h-6 rounded-full transition-colors relative ${form.is_pregnant === false ? 'bg-pink-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                                role="switch" aria-checked={form.is_pregnant === false}>
                                <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${form.is_pregnant === false ? 'translate-x-6' : ''}`} />
                            </button>
                        </div>
                    )}
